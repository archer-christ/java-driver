    ...
    public void operationComplete(Future<java.lang.Void> future) throws Exception {
      if (!future.isSuccess()) {
        ...
        if (result.isDone()) {
          ...
          cancel();
        } else if (scheduleNextExecution && isIdempotent) {
          int nextExecution = execution + 1;
          // Note that `node` is the first node of the execution, it might not be the "slow" one if
          // there were retries, but in practice retries are rare.
          long nextDelay =
              context

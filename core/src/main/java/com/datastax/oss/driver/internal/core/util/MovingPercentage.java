...
/**
 * Records how often a condition is met over time, with exponential decaying of older measurements.
 */
public class MovingPercentage {
  ...
  public MovingPercentage(
    ...
    this.valueRef = new AtomicReference<>(new Value(nanoTime(), initialPercentage));
  ...
  private class Value {
    ...
    Value update(long newTimestampNanos, boolean conditionWasMet) {
      ...
      double newPercentage = (conditionWasMet ? alpha * 100 : 0) + (1 - alpha) * percentage;
      boolean newHigh;
